...
class dpd_2015_Menukit {
	 ...
	 *
	 * @global 		 			$dpd_2015_themekit 			Themekit class
	 *
	 * @param 		string 		$item_output		//
	 * @param 		object 		$item				//
	 * @param 		int 		$depth 				//
	...
	public function menu_caret( $item_output, $item, $depth, $args ) {
		...
		$output .= $item->title;
		$output .= '<span class="children">' . $dpd_2015_themekit->get_svg( 'caret-down' ) . '</span>';
		$output .= '</a>';
	...
	public function get_svg_by_class( $classes ) {
		...
		foreach ( $classes as $class ) {
			...
			$check = $dpd_2015_themekit->get_svg( $class );
 ...
 */
$dpd_2015_menukit = new dpd_2015_Menukit();
